const { zenkaku2Hankaku, getTime } = require("../utils")
...
|なお、このレシートは架空の書店の
|レシートとなっております
|${year}年${month}月${day}日（${dayOfWeekStr}）　${hours}時${minutes}分

取引レシート
営業日　${year}年${month}月${day}日（${dayOfWeekStr}）
...
${items.map((item) => {
  ...
  return `
|書　　${item}
${Math.round(Math.random()*1e+11)}　${Math.ceil(Math.pow(Math.random(), .4)*5)}　　${val.toLocaleString()}|
  `
})}
-
|小　計　　　　　　　　　　　　　 4   ${sum.toLocaleString()}

|合　計（税込）　　　　　${zenkaku2Hankaku(sum.toLocaleString())}
|※内訳（税抜）　　　　　${zenkaku2Hankaku((sum * (10/11)).toLocaleString())}
|　　（消費税）　　　　　　${zenkaku2Hankaku((sum * (1/11)).toLocaleString())}
-
